...
var render = function(arr) {
  return arr.reduce(function(previousValue, currentValue, index, array) { 
    return previousValue +
	  ...
	  '<p>' + currentValue.content + '</p>'+'</li>'												
	},'');
}

var render2 = function(arr2) {
  return arr2.reduce(function(previousValue, currentValue, index, array) { 
    return previousValue +'<li><a onclick="loadpost('+index+')">'+currentValue.title+'</a></li>'},'');
}
function loadpost(index){
  $('.liPaginate').html('<li id ='+index+'>'+
  '<h4>'+posts[index].title+'</h4>'+
  '<p>' + posts[index].date + '</p>'+
  '<p>' + posts[index].content + '</p>'+'</li>')
}
function loadbtn(){
  ...
  $('.liPaginate').html(render(filterlist));
}
